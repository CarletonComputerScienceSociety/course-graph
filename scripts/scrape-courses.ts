...
export function buildCourseData(
  course: ReturnType<cheerio.CheerioAPI>,
): Course {
  ...
  if (course.text().includes('Precludes additional credit for')) {
    ...
    if (infoHtml) {
      prereqRaw = clean(
        cheerio
          .load(infoHtml.split('Prerequisite(s):')[1].split('<br')[0])
          .text(),
      );
    }
  }
...
export async function getProgramCourses(
  ...
  for (const program of Object.values(programs) as Program[]) {
    const programHtml = await cheerio.fromURL(
      program.url + '#coursesinventory',
    );
...
export async function scrape(): Promise<void> {
  const programsHtml = await cheerio.fromURL(
